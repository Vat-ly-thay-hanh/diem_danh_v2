...
async function saveAttendance() {
    ...
    document.getElementById("dlgLoading").showModal();

    const students = studentManager.map(student => {
        switch (student.state) {
    ...
    try {
        ...
        const result = await response.json();
        document.getElementById("dlgLoading").close();
        if(result.success){
            document.getElementById("dlgSuccess").showModal();
        }else{
